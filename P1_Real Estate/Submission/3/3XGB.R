...
View(house[grep("/",house$Address),])

house$Landsize[grep("/",house$Address)]=house$BuildingArea[grep("/",house$Address)]



house$BuildingArea[house$BuildingArea<30]=121

plot(house$tot_room,house$BuildingArea)


house$Type[grep("/",house$Address)]="n"

house$Distance=1/house$Distance

house$totR_D=house$tot_room*house$Distance
...
set.seed(2)
bst.cv = xgb.cv(param=param,  xgbd ,nrounds=3000 ,early_stopping_rounds=100,nfold = 4,seed=27)#,"maximize"=F,niter=10,  ,niter=500, nrounds = 50,colsample_bytree =0.9,gamma =100,min_child_weight=10,subsample =0.9)#)#prediction=TRUE, verbose=FALSE,
...
set.seed(2)
bst <- xgboost(param=param,xgbd ,nrounds=226 ,nfold = 4,seed=27,verbose = 0)#,niter=299, early_stopping_rounds=50,min_child_weight=10,subsample =0.9)#
...
set.seed(2)
btr$pred=predict(bst,xgbd,ntreelimit = 226)
...
bt=b_test[1:1000,]
set.seed(2)
bt$pred=predict(bst,xgbdtest,ntreelimit = 226)


res=bt$Price-bt$pred
plot(bt$Distance,bt$pred)
a=data.frame(Price=bt$Price,pred=bt$pred,res=res,tp=bt$Type_h)
ggplot(a,aes(Price,res))+geom_point()+geom_smooth()
ggplot(a,aes(Price,pred))+geom_point()+geom_smooth()


rmse_test=sqrt(mean((a$res[a$Price<1000000 & a$tp<0.82])^2))#
rmse_test
plot(log(bst.cv$evaluation_log[,4]),type = "l")
...
ggplot(bt,aes(Type_h,res))+geom_point()+geom_smooth()

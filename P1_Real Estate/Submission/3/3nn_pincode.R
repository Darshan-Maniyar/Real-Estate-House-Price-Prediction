...
length(table(house$Postcode))
length(table(house$Suburb))
length(table(house$Distance,house$CouncilArea))
# View(table(house$Distance,house$CouncilArea))
# View(house)
# ggplot(house,aes(Price))+geom_histogram(binwidth=10)
...
b=dummy(b,c("Type","Method"),limit = 200,rm_original = T)
...
# b[-a]=scale(b[-a])
set.seed(2)
a1=sample(1:nrow(b), nrow(b)*0.80)
b_train=b[a1,]
b_test=b[-a1,]
str(b_train)
...
set.seed(1)
nn <- nnet(Price/3500000~.,data=b_train,size = 30,decay=0.01, maxit=2000,linout=T)#,subset = sampidx
summary(nn)
plot(nn)

...
for (i in 0:10) {
  assign(paste("fit_h", i, sep=""), cv.glmnet(x.b_train, y.b_train, type.measure="mse", 
                                            alpha=i/10,family="gaussian"))
}


yhat0 <- predict(fit_h0, s=fit_h0$lambda.1se, newx=x.b_train)
yhat1 <- predict(fit_h1, s=fit_h1$lambda.1se, newx=x.b_train)
yhat2 <- predict(fit_h2, s=fit_h2$lambda.1se, newx=x.b_train)
yhat3 <- predict(fit_h3, s=fit_h3$lambda.1se, newx=x.b_train)
yhat4 <- predict(fit_h4, s=fit_h4$lambda.1se, newx=x.b_train)
yhat5 <- predict(fit_h5, s=fit_h5$lambda.1se, newx=x.b_train)
yhat6 <- predict(fit_h6, s=fit_h6$lambda.1se, newx=x.b_train)
yhat7 <- predict(fit_h7, s=fit_h7$lambda.1se, newx=x.b_train)
yhat8 <- predict(fit_h8, s=fit_h8$lambda.1se, newx=x.b_train)
yhat9 <- predict(fit_h9, s=fit_h9$lambda.1se, newx=x.b_train)
yhat10 <- predict(fit_h10, s=fit_h10$lambda.1se, newx=x.b_train)
...
plot(exp(c(yhat2)) ~ exp(c(y.b_train)))
sqrt(mean((exp(c(yhat2)) - exp(c(y.b_train)))^2))

plot(fit_h2, xvar="lambda")
hist(yhat0)

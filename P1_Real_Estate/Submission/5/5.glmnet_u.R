...
for (i in 0:10) {
  assign(paste("fit_u", i, sep=""), cv.glmnet(x.b_train, y.b_train, type.measure="mse", 
                                              alpha=i/10,family="gaussian"))
}



yhat0 <- predict(fit_u0, s=fit_u0$lambda.1se, newx=x.b_train)
yhat1 <- predict(fit_u1, s=fit_u1$lambda.1se, newx=x.b_train)
yhat2 <- predict(fit_u2, s=fit_u2$lambda.1se, newx=x.b_train)
yhat3 <- predict(fit_u3, s=fit_u3$lambda.1se, newx=x.b_train)
yhat4 <- predict(fit_u4, s=fit_u4$lambda.1se, newx=x.b_train)
yhat5 <- predict(fit_u5, s=fit_u5$lambda.1se, newx=x.b_train)
yhat6 <- predict(fit_u6, s=fit_u6$lambda.1se, newx=x.b_train)
yhat7 <- predict(fit_u7, s=fit_u7$lambda.1se, newx=x.b_train)
yhat8 <- predict(fit_u8, s=fit_u8$lambda.1se, newx=x.b_train)
yhat9 <- predict(fit_u9, s=fit_u9$lambda.1se, newx=x.b_train)
yhat10 <- predict(fit_u10, s=fit_u10$lambda.1se, newx=x.b_train)
...
plot(exp(c(yhat7)) ~ exp(c(y.b_train)))
sqrt(mean((exp(c(yhat7)) - exp(c(y.b_train)))^2))
